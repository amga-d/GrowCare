...
class EditProfileViewModel @Inject constructor(
    ...
    private val authDataSource: com.example.growCare.data.remote.firebase.FirebaseAuthDataSource
) : ViewModel() {
        ...
        viewModelScope.launch {
            try {
                ...
                if (user != null) {
                    ...
                    println("EditProfile: No user found, creating from auth")
        ...
        viewModelScope.launch {
            try {
                ...
                if (firebaseUser != null) {
                    ...
                    println("EditProfile: Creating new profile from auth - ${newUser.email}")
                    
                    // Save to repository (will save to both Firestore and Room)
                    saveUserProfileUseCase(newUser)
                        .onSuccess {
                            println("EditProfile: Profile created successfully")
                            // The collectLatest above will pick up the new profile
                        }
                        .onFailure { e ->
                            println("EditProfile: Failed to create profile - ${e.message}")
                            _events.emit(EditProfileEvent.ShowError("Could not create profile: ${e.message}"))
                        }
                } else {
                    println("EditProfile: No Firebase user found")
                    _events.emit(EditProfileEvent.ShowError("Could not load profile. Please ensure you're logged in."))
                }
            } catch (e: Exception) {
                println("EditProfile: Exception creating profile from auth - ${e.message}")
